...
export class EspejoEnMemoria implements AlmacenDeEspejo {
  ...
  private idTecnicoGuardado: string | null = null;
  private idBodegaGuardada: string | null = null;
  ...
  async reemplazar(jornada: JornadaDelDispositivo): Promise<void> {
    ...
    this.idTecnicoGuardado = jornada.idTecnico;
    this.idBodegaGuardada = jornada.bodega?.id ?? null;
  }
  ...
  async anotarEvidenciaLocal(idOrden: string, clave: string): Promise<void> {
    this.ordenesGuardadas = this.ordenesGuardadas.map((orden) =>
      (orden.id === idOrden && !orden.evidenciasRegistradas.includes(clave)
        ? { ...orden, evidenciasRegistradas: [...orden.evidenciasRegistradas, clave] }
        : orden));
  }
  ...
  async identidad(): Promise<{ idTecnico: string | null; idBodega: string | null }> {
    return { idTecnico: this.idTecnicoGuardado, idBodega: this.idBodegaGuardada };
  }
}
