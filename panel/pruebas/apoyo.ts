/**
 * Apoyo de las pruebas del panel.
 *
 * Dos familias de dobles: un `fetch` de mentira para el cliente de la API, y
 * un servidor simulado con la semantica real del protocolo de campo
 * —idempotencia por clave, confirmacion explicita, cargas reanudables— para
 * el motor de sincronizacion.
 */
import type {
  EstadoDeCarga, JornadaDelDispositivo, OperacionEnCola, PeticionIniciarCarga,
  ResultadoOperacion, ResultadoSincronizacion, Sesion, UsuarioAutenticado,
} from '@servitotal/compartido';
import { ESTADO_OPERACION } from '@servitotal/compartido';
import { ErrorDeApi } from '../src/api/cliente.js';
import type { ClienteDeCampo, DetectorDeConexion } from '../src/campo/motor.js';
 ...
 */
export class ServidorSimulado implements ClienteDeCampo {
